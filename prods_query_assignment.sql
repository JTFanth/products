...
SELECT invoice_num FROM invoice where invoice_date = '13-SEPT-07' and cust_id = '1619';
...
SELECT cust_id, cust_name FROM customer WHERE SUBSTRING(cust_name, 1, 1) = 'S';
...
SELECT COUNT(cust_name) FROM customer WHERE cust_balance < cust_limit;
...
SELECT MIN(cust_balance) AS min_customer_balance FROM customer;
...
SELECT rep_id, SUM(cust_balance) FROM customer GROUP BY rep_id;
...
SELECT rep_id, SUM(cust_balance) FROM customer GROUP BY rep_id HAVING SUM(cust_balance) > 12000;
